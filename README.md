...
TPO 是一种在推理阶段动态调整语言模型输出的 AI 优化框架。
1. **核心功能**：通过将奖励信号转化为文本反馈，动态调整模型输出，使其更符合人类偏好。
2. **技术原理**：基于“文本损失”生成“文本梯度”，指导模型逐步改进输出，无需更新模型参数。

## TPO 是什么

`TPO（Test-Time Prompt Optimization）`是一个用于优化大语言模型（LLM）输出文本的框架，它通过使用奖励模型和迭代反馈来指导基础模型生成结果更好的文本。

这里以 `DeepSeek-R1-Distill-Qwen-14B` 作为基础模型进行文本输出，并且提供了一个友好的 WebUI 让你更方便的使用它来进行实时优化基础模型，它会根据你的输入数据，对每个输入进行多轮评估和迭代反馈，使得基础模型在推理过程中与人类偏好对齐，生成更好的结果。
...
这里我**推荐配置 4090 4卡**的运行环境，选用 DeepSeek 的`DeepSeek-R1-Distill-Qwen-14B` 模型作为基础模型来获取更有性价比的运行结果，下面的步骤都以该基础模型为例，如有出入请自行按说明修改参数配置。

# 如何运行 TPO

## 1. 运行环境准备（通过镜像一键部署实例可跳过该步骤，实例已配置好运行环境）

虚拟环境及其所需的依赖均已在实例内配置完成，你可以直接跳过该步骤继续运行，本地部署按照以下步骤配置运行环境：
...
## 2. 下载基础模型和奖励模型（通过镜像一键部署实例可跳过该步骤，镜像已内置模型）

运行需要的模型均在镜像内已内置，你可以直接跳过该步骤继续运行，本地部署则需要手动下载。
...
到对应的 HuggingFace 仓库下载模型并放到指定的路径中`（/model/HuggingFace/...）`。
...
一般情况下 TPO 框架建议在单机单GPU环境中运行。若服务器资源足够，你也尝试可以在同一环境中运行 TPO，输入以下指令来确定当前系统配置的GPU数量及显存分配情况：
```bash
watch -n 1 nvidia-smi
```

在多 GPU 且资源有限的环境下，TPO 默认让 Pytorch 自动分配模型到多个 GPU 上，但假设分配到了一个正在被其他模型使用的 GPU 上时，则可能会发生显存溢出的情况，导致程序报错而停止运行。所以在这种多GPU的环境中，你需要确保 TPO 使用的 GPU 设备是空闲的，并且可以分配给 TPO 脚本使用。
...
## 运行方式一：启动 TPO WebUI，提供了友好交互的 TPO 优化平台（实时优化和展示结果）

WebUI 服务默认通过 `7860` 端口进行访问，详细可查看`gradio_app.py`，镜像已经配置了端口转发。
...
- 先在`模型设置`初始化模型，连接 vLLM 服务器和奖励模型。

![tpo_model_init.png](https://s2.loli.net/2025/02/13/qBN1A4IM78HWcih.png)

- 模型初始化成功后，就可以到`优化设置`页尽情体验啦！输入问题后点击`开始优化`按钮即可。

![tpo_result.png](https://s2.loli.net/2025/02/13/VCidEf1m8Wv2GOS.png)

## 运行方式二：手动执行 TPO 优化框架脚本（类似预训练的优化方式）

在终端运行以下命令以执行 TPO 脚本，该脚本会先加载一个奖励模型（预计耗时1～2分钟）与作为 vLLM 服务器部署的策略模型进行交互：
...
1. 预处理阶段 ：
   - 你需要先运行这个 `run.py` 脚本
   - 脚本会读取你的输入数据（通过 --data_path 参数指定的 JSON 文件）
   - 对每个输入进行多轮优化
2. 优化过程 ：
   - 对每个输入查询，会进行 max_iterations 次迭代（默认2次）
   - 每次迭代会：
     - 生成多个候选回答（由 sample_size 参数控制，默认5个）
     - 使用奖励模型评估这些回答
     - 选择最佳回答并优化提示
3. 结果保存 ：
   - 优化后的结果会保存在输出文件中
...
所以，这不是一个实时优化的过程，而是需要你先运行这个框架完成优化，然后再使用优化后的结果。

# TPO 的迭代细节

TPO 优化过程的每次迭代捕获**四个关键项**，均与作为基础模型的大语言模型直接相关：

- 文本损失计算过程中的输入和输出，比较所选和被拒绝的响应。
- 用于生成梯度的输入。
- 作为文本梯度的输出。
- 用于下一轮响应生成的迭代优化输入。

运行脚本后，将在`logs/`目录下生成日志文件，以JSON格式存储，你可以详细地跟踪每次迭代的优化过程。